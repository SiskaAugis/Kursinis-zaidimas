...
class MainMenu(Entity):
    def __init__(
        ...
        quit_yes = Button(
            text="Yes", color=color.black, scale_y=0.1, scale_x=0.3, y=0.05, parent=self.quit_menu
        )
        quit_no = Button(
            text="No", color=color.black, scale_y=0.1, scale_x=0.3, y=-0.07, parent=self.quit_menu
        )
        ...
        back_button_singleplayer = Button(
            text="Back", color=color.black, scale_y=0.1, scale_x=0.3, y=-0.34, parent=self.main_menu
        )
        ...
        def reset_highscore():
            self.car.reset_highscore()

        reset_highsore_button = Button(text = "Reset Highscore", color = color.black, scale_y = 0.1, scale_x = 0.3, y = 0.1, parent = self.gameplay_menu)
        reset_highsore_button.on_click = Func(lambda: self.car.reset_highscore())  # Corrected line
    ...
    def update(self):
        if not any(menu.enabled for menu in self.menus):
        ...
        # Set the camera's position and make the car rotate for menus
        if self.start_menu.enabled or self.garage_menu.enabled or self.quit_menu.enabled:
            ...
            # Camera behavior when no menu is enabled (during gameplay)
        ...
        # Removed multiplayer-specific username saving logic
    ...
    def input(self, key):
        ...
        if not any(menu.enabled for menu in self.menus):
            ...
            # Disable game-specific UI elements when a menu is open
            self.car.timer.disable()
            self.car.reset_count_timer.disable()
        ...
        # Control car normals based on menu state
        if self.start_spin:
