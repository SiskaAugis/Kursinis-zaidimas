# File: c:\Users\augus\Desktop\Rally-master\ai.py
from ursina import *
    ...
    def update(self):
        ...
        if self.y_ray.distance <= self.scale_y * 1.7 + abs(movementY):
            ...
            if not self.hitting_wall:
                ...
                # Corrected line: Set the heading (Y rotation) using hpr
                # OLD LINE: self.rotation_parent.rotate((0, self.rotation_y + 180, 0))
                self.rotation_parent.hpr = (self.rotation_y + 180, self.rotation_parent.hpr[1], self.rotation_parent.hpr[2])
            else:
